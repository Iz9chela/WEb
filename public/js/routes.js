...
function createHtml4opinions(targetElm){
    ...
    if(opinionsFromStorage){
        ...
        opinions.forEach(opinion => {
            opinion.created = (new Date(opinion.created)).toDateString();//sort po date dobavleniya
            opinion.willReturn = 
    ...
    document.getElementById(targetElm).innerHTML = Mustache.render(
        document.getElementById("template-opinions").innerHTML,//obnovlayem 4erez mustache
        opinions
        );
}
...
function fetchAndDisplayArticles(targetElm){
    ...
    .then(response =>{
        if(response.ok){//if ok - return json file s articlami
            return response.json();
        }else{//Mu sozdaem oshibky
    ...
    .then(responseJSON => {//perenosim dannie s json v mustache, renderim i vstavlaem v template article
    ...
    .then( ()=> {//bez parametra
        let contentRequests = articleList.map(//sozdaem masiv zaprosov na server 
            article => fetch(`${serverUrl}/${article.id}`)
        );

        return Promise.all(contentRequests);//Try to resolve all promises
    })
    .then(responses =>{//to 4to promise all vernet,zapishet'sa v responses
        let failed="";
    ...
    .then(articles => {
        articles.forEach((article,index) =>{//V kazdiy article add content 
   ...
   .then( () =>{
    ...
    function renderArticles(articles) {
        document.getElementById(targetElm).innerHTML =Mustache.render(document.getElementById("template-articles").innerHTML, articles); //write some of the response object content to the page using Mustache
        // console.log('KU');
